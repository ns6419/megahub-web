...
<body>
    ...
    <script>
        ...
        async function sendAIChat() {
            const input = document.getElementById('aiInputField'); const chatBox = document.getElementById('aiChatBox'); const btn = document.getElementById('aiSendBtn'); const query = input.value.trim(); if(!query) return;
            const userMsg = document.createElement('div'); userMsg.className = 'msg user'; userMsg.textContent = query; chatBox.appendChild(userMsg); input.value = ''; chatBox.scrollTop = chatBox.scrollHeight; btn.disabled = true;
            try {
                const res = await fetch('/api/ask-ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: query }) });
                const data = await res.json();
                const botMsg = document.createElement('div'); botMsg.className = 'msg bot'; botMsg.textContent = data.reply; chatBox.appendChild(botMsg);
            } catch {
                const errMsg = document.createElement('div'); errMsg.className = 'msg bot'; errMsg.textContent = 'TRANSMISSION FLUCTUATION.'; chatBox.appendChild(errMsg);
            } finally { btn.disabled = false; chatBox.scrollTop = chatBox.scrollHeight; }
        }
...
app.post('/api/ask-ai', (req, res) => {
    ...
    aiReq.on('error', () => res.json({ reply: "Transmission failure on the AI channel." }));
...
app.post('/submit-ticket', (req, res) => {
    ...
    const nreq = https.request(opt, (nres) => { res.json({ success: true }); });
